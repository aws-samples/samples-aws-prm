#!/usr/bin/env bash
#
# Deploy the PRM attribution sidecar sample to an EKS Auto Mode cluster.
#
# Prerequisite: deploy sidecar-stack.yaml first. That CloudFormation stack
# creates the cluster, the dedicated sidecar IAM role, and the EKS Pod Identity
# association for the sidecar service account.
#
# This script:
#   1. Builds the sidecar image and pushes it to ECR.
#   2. Applies the namespace, service account, and sample Deployment (with the
#      native sidecar) to the cluster.
#
# Usage:
...
#
# Options:
#   --cluster NAME          EKS cluster name      (default: prm-sidecar)
#   --region REGION         AWS region            (default: from AWS config, else eu-west-1)
#   --profile PROFILE       AWS CLI profile       (default: none)
#   --namespace NS          Kubernetes namespace  (default: prm-sidecar)
#   --repo NAME             ECR repository name   (default: prm-sidecar)
...
#
# NOTE: --namespace must match the SidecarNamespace used for sidecar-stack.yaml,
#       or Pod Identity will not grant credentials.
#
set -euo pipefail
...
# Defaults
CLUSTER_NAME="prm-sidecar"
REGION=""
PROFILE=""
NAMESPACE="prm-sidecar"
REPO_NAME="prm-sidecar"
IMAGE_URI=""
PRODUCT_CODE=""
...
if [ -z "${IMAGE_URI}" ]; then
  ...
  # The ECR repository is created by the CloudFormation stack (sidecar-stack.yaml),
...
echo "==> Applying sample Deployment with sidecar"
RENDERED="$(mktemp)"
trap 'rm -f "${RENDERED}"' EXIT
sed -e "s#__SIDECAR_IMAGE__#${IMAGE_URI}#g" \
...
echo
echo "==> Done."
echo "Watch the sidecar attribute the node it landed on:"
echo "    kubectl -n ${NAMESPACE} logs deploy/sample-app -c prm-sidecar -f"
